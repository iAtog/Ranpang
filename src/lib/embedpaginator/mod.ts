...
// source: https://github.com/Spimy/embed-paginator/
...
export class PaginatedEmbed {
    ...
    private embedMsg: Message | undefined;
    ...
    private async setupPages(items: EmbedItems) {
        ...
        while (colours.length > 0 || descriptions.length > 0 || fields?.length > 0) {
            ...
            pages.push(page as unknown as any);
        }
    ...
    private async changePage() {
        ...
        if (this.options.urls) {
            this.messageEmbed.setURL(this.pages[this.currentPage - 1].urls![0] || null);
        }
    ...
    public async send({ message, options: { interaction, ephemeral, followUp, channel, components } }: SendOptions) {
        ...
        if (interaction) {
            if (interaction.isRepliable()) {
                ...
                throw new Error(`The interaction ${(interaction as any).id} passed as argument cannot be replied to.`);
            }
        } else {
